...
integer,parameter				::n=150,unit=1
integer					::i,j,k,iz0,kontagailua,itx,ity,itz
real(kind=dp),dimension(:,:,:),allocatable	::p_orain,p_berria,u_orain,u_berria,v_orain,v_berria,w_orain,w_berria,oztoporik
real(kind=dp)				::rho,rv,rp,x,y,tau,t,pt,L,zeta_sabaia,kont_max
real(kind=dp)				::zn_leiho,zn_arbela,zn_horma,zn_lurra,zn_sabaia,zn_xurgakor
...
do i=0,nx
	...
	!MB presioarentzat 
	p_berria(0,:,:)=u_orain(0,:,:)*zn_xurgakor
	p_berria(nx,:,:)=u_orain(nx+1,:,:)*zn_xurgakor
	p_berria(:,0,:)=v_orain(:,0,:)*zn_xurgakor
	p_berria(:,ny,:)=v_orain(:,ny+1,:)*zn_xurgakor
	p_berria(:,:,0)=w_orain(:,:,0)*zn_xurgakor
	p_berria(:,:,nz)=w_orain(:,:,nz+1)*zn_xurgakor
	
	!oztopoak
	do i=1,zenbat_oztopo
			call mb_p_oztopo(p_berria,u_orain,v_orain,w_orain,zn_xurgakor,ind(obloke(i,1)),ind(obloke(i,2)),ind(obloke(i,3)),ind(xbloke(i)),ind(ybloke(i)),ind(zbloke(i)))
	end do
	...
	!p-n falta diren puntuak ezartzeko: onartu p(n+1)~p(n+2)
	!X norabideko MB
	...
	do i=1,zenbat_oztopo
		call mb_u_oztopo(p_berria,u_berria,v_berria,w_berria,zn_xurgakor,ind(obloke(i,1)),ind(obloke(i,2)),ind(obloke(i,3)),ind(xbloke(i)),ind(ybloke(i)),ind(zbloke(i)))
	end do
