...
real(kind=dp)				::rho,rv,rp,x,y,tau,t,pt,L,zeta_zapaia,kont_max
real(kind=dp)				::zn_xurgakor
...
itz=ind(iturriz)
pt=p(t)
p_orain(itx,ity,itz)=pt	!eta hasierako abiadura=0
...
do
	...
	!X norabideko MB
	u_berria(0,:,:)=p_berria(0,:,:)/zn_xurgakor
	u_berria(nx+1,:,:)=p_berria(nx,:,:)/zn_xurgakor

	!Y norabideko MB
	v_berria(:,0,:)=p_berria(:,0,:)/zn_xurgakor
	v_berria(:,ny+1,:)=p_berria(:,ny,:)/zn_xurgakor

	!Z norabideko MB
	w_berria(:,:,0)=p_berria(:,:,0)/zn_xurgakor
	w_berria(:,:,nz+1)=p_berria(:,:,nz)/zn_xurgakor
